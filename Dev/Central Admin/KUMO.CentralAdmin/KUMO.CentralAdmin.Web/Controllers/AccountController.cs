...
namespace KUMO.CentralAdmin.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, Company = model.Company, FirstName = model.Firstname, LastName = model.Lastname, Status = UserStatus.TemporaryPassword.ToString() };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    var roleresult = UserManager.AddToRole(user.Id, RoleName.Admin);
        ...
        public ActionResult ResetPassword(string temp, string code = "")
        {
            ...
            var user = userService.GetUserById(temp);
        ...
        public ViewResult Index(int page = 1, int pagesize = 20, string orderby = "Id", string keyword = "", string order = "desc")
        {
            ...
            var accounts = userService.GetAllUsers().OrderBy(orderByExpression);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,Firstname,Lastname,Company,Phone")] UserViewModel userModel)
        {
            if (ModelState.IsValid)
            {
                var user = new User();
