	...
	public class ExcelRangeBase : ExcelAddress, IExcelCell, IDisposable, IEnumerable<ExcelRangeBase>, IEnumerator<ExcelRangeBase>
	{
        ...
        private bool PropertyHasAttribute<T>(string properyName, Type attributeType)
        {
            ...
            if (att != null)
            {
                ...
                var type = ResolveType(att.MetadataClassType.UnderlyingSystemType.FullName);///Type.GetType(att.MetadataClassType.UnderlyingSystemType.FullName);
        ...
        public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, string[] fields,bool PrintHeaders = true, TableStyles TableStyle= TableStyles.Light1)
        {
            List<MemberInfo> mi = new List<MemberInfo>();
            var type = typeof(T);
            foreach (var item in fields)
	            {
		            var methodInfo = type.GetProperty(item);
                    if (methodInfo != null)
                    {
                        if (item != null && !PropertyHasAttribute<T>(methodInfo.Name, typeof(ExcelIgnoredAttribute)))
                        {
                            mi.Add(methodInfo);
                        }
                    }
            ...
            return LoadFromCollection(Collection, PrintHeaders, TableStyle, BindingFlags.Public | BindingFlags.Instance, mi.ToArray());
        }
		...
		public ExcelRangeBase LoadFromCollection<T>(IEnumerable<T> Collection, bool PrintHeaders, TableStyles TableStyle, BindingFlags memberFlags, MemberInfo[] Members)
		{
			...
			if (Members == null)
			{
				...
				var allmembers = type.GetProperties(memberFlags);
                ...
                foreach (var item in allmembers)
	            {
                    ...
                    var ignored = PropertyHasAttribute<T>(item.Name, typeof(ExcelIgnoredAttribute));
                    var att = item.GetCustomAttributes(typeof(ExcelIgnoredAttribute), true);

                    if (ignored  || (att != null && att.Length > 0) || isVirtual)
                    {
                        continue;
                    }
                    list.Add(item);
				...
				foreach (var item in Collection)
				{
                    ...
                    else
                    {
                        foreach (var t in Members)
                        {
                            ...
                            {
                                var pi = (PropertyInfo) (t);
                                if (pi.PropertyType == typeof(DateTime) || pi.PropertyType == typeof(DateTime?))
                                {
                                    var dateTime = Convert.ToDateTime(((PropertyInfo)t).GetValue(item, null)) ;

                                    _worksheet.Cells[row, col++].Value = dateTime.ToString("dd/MM/yyyy");
                                }
                                else
                                {
